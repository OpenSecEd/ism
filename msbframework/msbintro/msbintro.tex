...
\title[Intro infosäk]{%
  Introduction to\\
  Swedish Civil Contingencies Agency (MSB) methodological support for
  introducing \ac{isms}
  \footnote{Lecture is based upon the materials provided by
    MSB~\cite{msb_metodstod} and previous lectures written by Carina Bengtsson
    and Daniel Bosk}.
}
...
\begin{frame}{Why protect the information?}{What is information security?}
...
\begin{frame}{Overview}{\insertsubsectionhead}
  \begin{itemize}
    \item Analyse the external contexts~\note{Omvärldsanalys} and the organisation.
